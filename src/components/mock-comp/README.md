...
This folder contains mock versions of Polar components that allow you to demo the pricing functionality without requiring Polar API keys.

## Purpose

These mock components are designed for:

- **Development**: Work on your app without setting up Polar
...
- Displays pricing plans with the same styling as the real Polar version
- Supports monthly/annual toggle
- Shows savings calculations
- Buttons show demo alerts instead of redirecting to Polar checkout
...
- Simulates Polar checkout buttons
...
### To Enable Real Polar Components

1. **Set up Polar API keys** (see `docs/POLAR_SETUP.md`)
...
Once you've set up Polar and switched to real components, you can safely delete this entire `mock-comp` folder.
...
- **Visual Feedback**: Demo badges clearly indicate this is not connected to real Polar

## Styling

All mock components use the exact same Tailwind classes and component structure as the real Polar components, ensuring a seamless transition when you're ready to switch to the real implementation.
